(ns anglican.smh
  ...
  (:use [anglican.state :exclude [initial-state]]
        anglican.inference
        [anglican.runtime :only [observe* sample* tag get-tag]]))
...
;;   {choice-id value log-p cont stochastic}
;; where
...
;;   - `stochastic' is true when the entry is stochastic.

(defrecord entry [choice-id value log-p cont stochastic])
...
(defn record-choice
  ...
  [state choice-id value log-p cont stochastic]
  (update-in state [::trace]
             conj (->entry choice-id value log-p cont stochastic)))
...
(defmethod infer :smh [_ prog value & {}]
  (letfn
    [(sample-seq [state]
       (lazy-seq
         (let [;; Choose uniformly a random choice to resample.
               ...
               state (if (or
                           (and (:stochastic entry)
                                ;; Always accept stochastic entries when
                                ;; they are consistent.
                                (> (utility next-state) (/ -1. 0.)))
                           (accept? (utility next-state) (utility prev-state)))
                       next-state
                       state)]
